...
| 공통 재생 대기 | 기본 **600ms**, 600–2000ms/200ms 단위 | 소스가 `delayMs`로 수신기에 전달 |
...
| 폰→PC | `{"type":"modeB","action":"start","sampleRate":48000,"channels":1,"codec":0,"delayMs":600}` | "폰 오디오를 보낼 테니 재생하라". `delayMs`는 소스가 지시하는 공통 동기 재생 지연(§7), 600–2000ms/200ms 단위 |
...
| 시작→응답 | `{"type":"cal","action":"prepare","id":"<uuid>","holdMs":350}` | 물리 음향 보정 준비. 스트림이 꺼져 있고 한 기기만 연결됐을 때 사용 |
| 응답→시작 | `{"type":"cal","action":"ready","id":"<uuid>"}` | 마이크가 1.8kHz 요청음을 들을 준비가 됨 |
| 응답→시작 | `{"type":"cal","action":"complete"\|"error","id":"<uuid>","message":"..."}` | 3kHz 응답음 재생 완료 또는 보정 실패 |
...
- 수신측은 공통 목표 지연(기본 600ms, 600–2000ms/200ms 단위)만큼 기다린 뒤 타임스탬프에 맞춰 재생한다.
...
- 이 방식은 OS 믹서/HAL 버퍼 크기가 달라도 동일한 음향 시각을 목표로 하며, 장시간 재생 중 시계 속도
  ...
  (0–300ms)으로 보정한다. 공학 기준선 100ms와 안정성 우선 제품 초기값 600ms의 구분은
