...
AudioBridge v2.7부터 모든 네트워크 스피커는 소스 타임스탬프보다 **600ms 늦은 공통 시각**으로
시작한다. 사용자는 설정에서 **600–2000ms를 200ms 단위**로 선택할 수 있으며 선택값은 저장된다.
수신 기기별 수동 보정은 **추가 지연 0~300ms**만 허용한다.
음수를 없앤 이유는 이미 지나간 하드웨어 출력 시각으로 프레임을 보낼 수 없기 때문이다.

공개된 측정과 플랫폼 상한을 조합한 **공학 기준선은 100ms**다. 제품 초기값 600ms는 이 평균값을
주장하는 수치가 아니라, 가정용 Wi-Fi의 긴 꼬리 지터와 제조사 DSP·절전·스케줄러 변동을 넉넉히
흡수하는 안정성 우선값이다. 사용자는 끊김이나 시차가 남을 때 공통 목표를 한 단계씩 늘릴 수 있다.
...
| **공학 기준선 합계** | | **100ms** |
| **안정성 우선 추가 여유** | 혼잡·절전·제조사 DSP의 긴 꼬리 지연을 넉넉히 흡수 | **500ms** |
| **제품 초기값** | | **600ms** |
| **사용자 선택 상한** | 특수한 고지연 출력·혼잡 환경을 위한 단계형 여유 | **2000ms** |
...
마감 시각을 놓치면 폐기한다. AudioBridge의 단계형 공통 목표도 이 원칙을 따른다.

## 물리 음향 자동 맞춤

자동 맞춤은 네트워크 RTT만 재지 않는다. 시작 기기가 1.8kHz 시험음을 스피커로 내고 상대 기기가
마이크로 감지한 뒤 350ms를 기다려 3kHz 응답음을 낸다. 시작 기기의 마이크가 응답을 감지하면
`(왕복시간 - 350ms) / 2`로 양쪽 오디오 경로의 평균적인 물리 지연을 추정한다. 여기에 출력/DSP
변동 여유 400ms를 더하고, 부족하지 않도록 600–2000ms의 다음 200ms 단계로 올림한다.

이는 정확한 실험실 계측기가 아니라 실사용 보조 기능이다. 방의 반향, 자동 이득 조절, 소음 제거,
스피커 음량, 마이크 위치에 따라 오차가 생긴다. 그래서 실패하면 기존 값을 바꾸지 않으며 수동 선택과
기기별 추가 지연을 항상 남겨 둔다. 자동 맞춤은 한 번에 연결된 기기 한 대에 대해서만 수행한다.
...
초기값은 안정성 우선 환경용이다. 공유기 혼잡, 절전 모드, Bluetooth 출력, 제조사 DSP는 600ms보다도
큰 변동을 만들 수 있다. 이때 공통 재생 대기를 늘리되, Bluetooth/헤드폰은 자동 음향 맞춤에서 제외하고
본체 스피커·마이크로 먼저 측정하는 것을 권장한다.
