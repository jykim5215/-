...
AudioBridge v2.6부터 모든 네트워크 스피커는 소스 타임스탬프보다 **100ms 늦은 공통 시각**을
기본 재생 목표로 사용한다. 수신 기기별 수동 보정은 **추가 지연 0~300ms**만 허용한다.
음수를 없앤 이유는 이미 지나간 하드웨어 출력 시각으로 프레임을 보낼 수 없기 때문이다.

100ms는 특정 표준이 보장하는 절대값이 아니라, 공개된 측정과 플랫폼 상한을 조합한
보수적 공학 기본값이다.
...
마감 시각을 놓치면 폐기한다. AudioBridge의 100ms 공통 목표도 이 원칙을 따른다.
...
이 기본값은 일반 환경용이다. 공유기 혼잡, 절전 모드, Bluetooth 출력, 제조사 DSP는 100ms보다 큰
변동을 만들 수 있다. 다음 고도화 단계는 실기기 마이크 펄스 측정으로 각 출력 경로의 추가 지연을
자동 추정하는 캘리브레이션이다.
