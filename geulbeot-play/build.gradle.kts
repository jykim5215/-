...
val distributionZip = tasks.register<Zip>("packageDistribution") {
    ...
    // The Play copy has no version.json - that file is the self-update manifest, and self-update
    // is exactly what is removed here. The version comes from the app module instead.
    val appBuildFile = layout.projectDirectory.file("app/build.gradle.kts").asFile
    val version = Regex("""versionName\s*=\s*"([^"]+)"""")
        .find(appBuildFile.readText())?.groupValues?.get(1) ?: "0.0.0"
