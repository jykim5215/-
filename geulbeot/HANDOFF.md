...
**2026-09-11 — P0~P6 완료(core-hwp 엔진). P7(안드로이드 앱) 진행 중.**

| 항목 | 상태 |
|---|---|
| HANDOFF.md (P0) | ✅ |
| UI 스타일 후보 3종 (P1) | ✅ 제시 완료. **사용자 선택 결과: 세 후보 모두 채택하지 않고 "한글 앱을 그대로 따라가는, 색이 튀지 않는 무난한 사무용 앱"으로 결정.** 리본 탭 구조(서식·입력·쪽·보기)를 따르고 회색/흰색 기반에 절제된 오피스 블루만 사용. |
| core-hwp 유틸·모델 (P2) | ✅ |
| CFB 리더/라이터 (P3) | ✅ 9MB DIFAT 경로까지 왕복 검증 |
| HWP 5.0 리더 (P4) | ✅ |
| HWPX 리더/라이터 (P5) | ✅ |
| HWP 5.0 라이터 (P6) | ✅ 보존(preservation) 정책 포함 |
| 편집 엔진 / 찾기·바꾸기 | ✅ `api/DocumentEditor`, `api/DocumentSearch` |
| 단위 테스트 | ✅ **45건 전부 통과** (`./gradlew :core-hwp:test`) |
| 안드로이드 앱 (P7~P9) | ⬜ 진행 중 |
| 자체 업데이트 / 바로가기 (P10) | ⬜ 미착수 |
| 배포 패키지 / 보안 문서 (P11) | ⬜ 미착수 |
| GitHub에 마지막으로 올린 버전 | 0.9.0 (엔진) |
| 앱 내 업데이트 확인 기능 | 미구현 |

### 엔진 구현 중 확인된 중요한 사실 (이어받는 에이전트가 반드시 알아야 할 것)

1. **CFB 섹터 번호는 부호 없는 값이다.** `MAX_REG_SECT`(0xFFFFFFFA)를 부호 있는 Int로 비교하면 모든 유효 섹터가 걸러진다. 유효 섹터는 항상 양수이므로 `sect >= 0` 만으로 판별한다.
2. **8-WCHAR 컨트롤의 구조는 `[컨트롤 문자][정보 6개][컨트롤 문자]` 이다.** 확장 컨트롤은 그 정보 6개 중 앞 2개가 ctrl id(UINT32, 하위 16비트 먼저)이다. 이 구조를 잘못 잡으면 `PARA_CHAR_SHAPE` 위치가 전부 어긋난다.
3. **소스 파일에 제어문자 리터럴을 절대 넣지 말 것.** NUL이나 U+0005를 직접 쓰면 파일이 바이너리로 취급되고 패치·검색이 어긋난다. `Char(0)`, `String(CharArray(n))`, `Char(5)` 형태로 쓴다.
4. **조각 내기(slice)는 반드시 복사본을 반환해야 한다.** `Paragraph.normalise()`가 인접 텍스트를 제자리에서 합치므로, 같은 인스턴스가 앞뒤 조각에 함께 들어가면 텍스트가 중복된다.
5. **글자/문단 모양의 `reserved` 필드는 우리가 해석하지 않는 비트만 담아야 한다.** 전체 속성 워드를 그대로 담으면 같은 모양이 서로 다른 것으로 판정된다.
6. **`.hwp`로의 신규 저장은 구역 정의(`secd`)를 직접 만들어야 하는 유일한 지점**이며, 이 프로젝트에서 확신도가 가장 낮은 부분이다(`Hwp5Template`). 그래서 새 문서의 기본 저장 형식은 `.hwpx`다.

### 다음 할 일
1. `app` 모듈: Compose UI(문서함 · 편집기 · 리본 도구 모음 · 찾기 · 쪽 설정 · 문서 정보).
2. PDF/인쇄 내보내기.
3. GitHub 릴리즈 기반 자체 업데이트, 바로가기, 아이콘.
4. 배포 압축 + 보안 점검 문서 + README.

### 이어받는 에이전트를 위한 메모
- 코드를 건드렸으면 **`version.json`의 version과 changelog를 함께 올리고**, 이 표를 갱신할 것.
- 보고할 때 **보안 점검 결과 한 줄**(개선한 취약점 / 없으면 "없음")을 반드시 덧붙일 것.
- `core-hwp`는 Android API를 import하지 말 것. 이 모듈의 JVM 테스트 가능성이 이 프로젝트의 유일한 자동 검증 수단이다.
- 이 개발 컨테이너에서는 `dl.google.com`이 차단되어 있어 `:app`을 빌드할 수 없다. `settings.gradle.kts`가 Android SDK 유무를 보고 `:app`을 자동으로 건너뛴다.
