...
| CI 빌드 | ✅ GitHub Actions에서 APK 생성 (`.github/workflows/android.yml`) |
| GitHub에 마지막으로 올린 버전 | 1.0.0 |
| 앱 내 업데이트 확인 기능 | ✅ 앱 시작 시 하루 1회 자동 확인 + 설정에서 수동 확인 |

### 검증 상태 — 이어받는 에이전트가 가장 먼저 알아야 할 것

- **`core-hwp`는 JVM 단위 테스트 48건으로 검증됨.** 형식 왕복, 제어문자 처리, 서식 보존, 형식 변환,
  편집 연산, 안전 상한까지 포함.
- **`app` 모듈은 이제 컴파일된다.** 2026-09-11, GitHub Actions 빌드 #6에서 처음 성공했고 APK가
  나왔습니다(`.github/workflows/android.yml`). 이 개발 컨테이너에서는 여전히 빌드할 수 없습니다 —
  `dl.google.com`이 조직 egress 정책으로 막혀 Android SDK를 받을 수 없기 때문입니다. **`app`을
  고쳤으면 확인할 곳은 CI뿐입니다.** 브랜치에 밀면 자동으로 돕니다.
- **아직 실기기에서 실행된 적은 없습니다.** 컴파일이 된다는 것과 화면이 제대로 나온다는 것은 다른
  이야기입니다. 실제 한글 문서로 열기·편집·저장을 확인하는 일이 남아 있습니다.
...
8. **`var x` 옆에 `fun setX(...)`를 두면 JVM 시그니처가 충돌한다.** 코틀린이 프로퍼티 setter에
   이미 그 이름을 쓴다. `pageLayoutView` + `setPageLayoutView()`가 이걸로 컴파일을 막았다.
   `setPageLayout()`처럼 이름을 달리한다.
9. **설정값(SharedPreferences)은 Compose 상태가 아니다.** 테마·확대율처럼 화면에 영향을 주는 값은
   ViewModel에 상태로 미러링해야 바뀐 것이 반영된다.

### 다음에 할 만한 일
1. **실기기에서 실제 한글 문서로 확인** — 열기·편집·저장·PDF. 아직 아무도 해 보지 않았습니다.
2. **고정된 서명 키 넣기.** 지금은 저장소 시크릿이 없어 CI가 디버그 서명으로 빌드합니다. 빌드마다
   키가 달라지므로 **앱 내 업데이트가 기존 설치를 덮어쓰지 못합니다.** `ANDROID_KEYSTORE_BASE64`,
   `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD` 네 개를 저장소
   시크릿에 넣으면 워크플로가 알아서 릴리즈 서명으로 바꿔 빌드합니다.
3. 머리말·꼬리말, 각주·미주 편집 (현재는 읽어서 보존만 함).
4. 화면에서의 실제 쪽 나눔 표시 (현재는 PDF/인쇄에서만 적용).
5. 수식·글상자 등 개체 편집.

### Play 제출용 사본에 대해
`../geulbeot-play`는 Google Play 제출용 **사본**입니다. Play 정책상 자체 업데이트 기능을 걷어내고
targetSdk 36으로 올린 것 말고는 같은 코드입니다. **개발은 이 원본에서만 하고**, Play에 낼 일이
생기면 그때 사본에 반영하세요. 자세한 차이는 `../geulbeot-play/PLAY-BUILD.md`에 있습니다.
